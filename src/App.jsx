    ...
    <div className="card-enter">
      ...
      <div style={{display:"flex",justifyContent:"space-between",gap:16,alignItems:"flex-start",flexWrap:"wrap",marginBottom:14}}>
        <div>
          ...
          <div style={{color:theme.muted,fontSize:13}}>v12.1 : import URL avec proxy Netlify.</div>
        </div>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.48)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:120,padding:20}}>
      <div className="modal-pop" style={{width:"min(560px,100%)",background:theme.panel,border:`1px solid ${theme.border}`,borderRadius:24,boxShadow:theme.shadowStrong,padding:26,textAlign:"center"}}>
        ...
        <div style={{fontWeight:900,fontSize:28,marginBottom:6}}>Version 12.1</div>
...
export default function App() {
 const [themeName,setThemeName]=useState("light"); const [showWhatsNew,setShowWhatsNew]=useState(()=>localStorage.getItem("menuFamille_v12_1_seen")!=="yes"); const theme=themeName==="light"?themeLight:themeDark; const [tab,setTab]=useState("menus"); const [recipes,setRecipes]=useState(INITIAL_RECIPES);
 useEffect(() => {
 ...
 {showWhatsNew && <WhatsNewModal theme={theme} onClose={() => { localStorage.setItem("menuFamille_v12_1_seen","yes"); setShowWhatsNew(false); }} />}
 ...
 {tab==="menus"?<><div className="card-enter" style={{display:"flex",justifyContent:"space-between",gap:16,alignItems:"flex-start",flexWrap:"wrap",marginBottom:14}}><div><h1 style={{margin:"0 0 6px 0",fontSize:24}}>Menus de la semaine</h1><div style={{color:theme.muted,fontSize:13}}>v12.1 : import URL avec proxy Netlify.</div></div><div style={{display:"flex",gap:12,flexWrap:"wrap"}}><button className="micro-btn" onClick={()=>setTab("courses")} style={{padding:"12px 16px",borderRadius:14,border:`1px solid ${theme.border}`,background:theme.surface,color:theme.text,fontWeight:900,cursor:"pointer"}}>🛒 Voir les courses</button><button className="micro-btn" onClick={()=>setShowSuggest(true)} style={{padding:"12px 16px",borderRadius:14,border:`1px solid ${theme.accent}`,background:theme.accent,color:"#fff",fontWeight:900,cursor:"pointer"}}>💡 Suggérer un repas</button><button className="micro-btn" onClick={generateSmartWeek} style={{padding:"12px 16px",borderRadius:14,border:`1px solid ${theme.border}`,background:theme.surface,color:theme.text,fontWeight:900,cursor:"pointer"}}>⚡ Générer semaine</button><select value={visualFilter} onChange={e=>setVisualFilter(e.target.value)} style={{padding:"12px 16px",borderRadius:14,border:`1px solid ${theme.border}`,background:theme.surface,color:theme.text,fontWeight:800}}><option value="all">Filtre : tous</option><option value="favorites">Favoris</option><option value="locked">Verrouillés</option><option value="empty">Repas vides</option><option value="light">Légers</option><option value="balanced">Équilibrés</option><option value="single">Plats uniques</option></select><button className="micro-btn" onClick={()=>setShowHistory(v=>!v)} style={{padding:"12px 16px",borderRadius:14,border:`1px solid ${theme.border}`,background:theme.surface,color:theme.text,fontWeight:800,cursor:"pointer"}}>{showHistory?"Masquer l’historique":"Voir l’historique"}</button></div></div>
